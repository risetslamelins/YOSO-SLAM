...
namespace ORB_SLAM3
{
PanopticNet::PanopticNet(PanopticModel model_type, const std::string& config_path){
    ...
    switch(model_type) {
        case PanopticModel::YOSO:
        case PanopticModel::MASK2FORMER:
            this->mCardboardId = 83; 
            break;
        case PanopticModel::PANOPTIC_FPN:
        default:
            this->mCardboardId = 4;
            break;
    }

    this->py_module = PyImport_ImportModule("inference"); //config_path.c_str()
    ...
    this->py_class = PyObject_GetAttrString(this->py_module, (char*)"PanopticSegmenter");
    ...
    {
        PyObject *pargs  = Py_BuildValue("(s)", config_path.c_str());
        ...
        this->py_method =  PyObject_GetAttrString(this->py_instance, (char*)"run_inference"); 
        if (!py_check(this->py_method)) // check load py_method
        {
            py_error("python error: Cannot load method run_inference");
            exit (EXIT_FAILURE);
        }
...
Panoptic_image PanopticNet::GetPanoptic()
{
    ...
    if (PyList_Check(pValue)) 
    {
        ...
        for(Py_ssize_t i = 0; i < PyList_Size(pValue); i++) 
        {
            ...
            if(category_id==this->mCardboardId && !isThing){isThing=true;}
