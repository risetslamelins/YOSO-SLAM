    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    std::string model_config_path = "config/model_config.yaml";
    cv::FileStorage fsPanoptic(model_config_path.c_str(), cv::FileStorage::READ);
    
    if(!fsPanoptic.isOpened()) {
        cerr << "Failed to open Panoptic model config at: " << model_config_path << endl;
        exit(-1);
    }
    
    std::string model_name = fsPanoptic["Panoptic_Model"];
    PanopticModel model_type;

    if (model_name == "YOSO") {
        model_type = PanopticModel::YOSO;
    } else if (model_name == "Mask2Former") {
        model_type = PanopticModel::MASK2FORMER;
    } else {
        model_type = PanopticModel::PANOPTIC_FPN;
    }

    mpPanopticNet = new PanopticNet(model_type, model_config_path);
